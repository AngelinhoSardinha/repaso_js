...
function añadeMedia(alumnosJSONparseado) {
    ...
    for (let alumno in alumnosJSONparseado) {
        let nota = 0;
        let cuentaNotas = 0;
        for (let p of propiedades) {
            if (p.startsWith("nota")) {
                nota += alumnosJSONparseado[alumno][p];
                cuentaNotas++;
            }
        }
        alumnosJSONparseado[alumno]["notaMedia"] = nota / cuentaNotas;
    }
    return alumnosJSONparseado;
 }
console.log(añadeMedia(alumnosJSONparseado));

function pintarTabla(alumnosJSONparseado) {
    let propiedades = Object.keys(alumnosJSONparseado[0]);
    ...
    for (let alumno in alumnosJSONparseado) {
        codigoHTML += "<tr>";
        for (let p of propiedades) {
            codigoHTML += `<td>${alumnosJSONparseado[alumno][p]}</td>`;
        }
        codigoHTML += "</tr>";
    }
    codigoHTML += "</table>";
